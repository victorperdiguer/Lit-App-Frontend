...
const NotificationCard = (props) => {
  ...
  return (
    ...
    <div className="notification-card" onClick={toggleHiddenNotification}>
      ...
      <h2>From {reveal ? `${senderName} ${senderSurname[0]}` : 'who?'}</h2>
      <div className="notification-discover-what">
        {/* <button className="notification-show-more" >Show more</button> */}
        {!reveal ? (<button className="notification-discover-what-button" onClick={(event) => handlePayment(event)}>25 <GiGems/></button>) : null}
      </div>
    </div>
    {!hiddenNotification && (
      <div className="hidden-notification">
        <p className="hidden-emoji">{question && question.emoji}</p>
        <p className="hidden-question">{question && question.question}</p>
      </div>
    )}
    </div>
  )
